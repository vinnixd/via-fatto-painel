...
const IntegracoesContent = () => {
  ...
  const validateGTM = (value: string) => !value || GTM_PATTERN.test(value);
  const validateFBPixel = (value: string) => !value || FB_PIXEL_PATTERN.test(value);
  const validateGA = (value: string) => !value || GA_PATTERN.test(value);
  ...
  const handleChange = (field: keyof IntegrationConfig, value: string) => {
    ...
    if (field === 'google_analytics_id') {
      if (cleanValue && !validateGA(cleanValue)) {
        newErrors.ga = 'Formato inválido. Ex: G-XXXXXXXXXX';
      } else {
        delete newErrors.ga;
      }
    }
  ...
  const handleSave = async () => {
    ...
    if (config.gtm_container_id && !validateGTM(config.gtm_container_id)) newErrors.gtm = 'Formato inválido';
    if (config.facebook_pixel_id && !validateFBPixel(config.facebook_pixel_id)) newErrors.fb = 'Formato inválido';
    if (config.google_analytics_id && !validateGA(config.google_analytics_id)) newErrors.ga = 'Formato inválido';
  ...
  const integrations = [
    {
      ...
      helpText: 'ID do contêiner do GTM',
    ...
    },
    {
      id: 'fb',
      title: 'Meta Pixel',
      description: 'Acompanhe conversões no Facebook e Instagram.',
      ...
      <Card key={integration.id} className="overflow-hidden">
        <CardHeader className="pb-4">
          <div className="flex items-start gap-4">
            <div className="w-14 h-14 rounded-xl bg-white border border-border flex items-center justify-center p-2 shrink-0 shadow-sm">
              <img src={integration.logo} alt={integration.title} className="w-full h-full object-contain" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <a href={integration.docsUrl} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
        ...
        <CardContent className="pt-0">
          <div className="space-y-2">
            <Label htmlFor={integration.id} className="text-sm font-medium">ID da Integração</Label>
    ...
    <div>
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 space-y-6">
          ...
          {activeTab === 'marketing' && (
            <div className="space-y-6 animate-fade-in">
              {integrations.filter(i => i.category === 'marketing').map(renderIntegrationCard)}
            </div>
          )}
...
export default IntegracoesContent;
