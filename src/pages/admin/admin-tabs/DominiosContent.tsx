...
const DominiosContent = () => {
  ...
  const deleteDomainMutation = useMutation({
    mutationFn: async (domainId: string) => {
      const { error } = await (supabase as any).from('domains').delete().eq('id', domainId);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tenant-domains', tenantId] });
  ...
  if (!isOwnerOrAdmin) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>Apenas administradores podem gerenciar domínios.</AlertDescription>
      </Alert>
    );
  }

  const renderDomainTable = (domainList: Domain[], title: string, description: string) => (
    <Card>
      <CardHeader>
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="text-center py-8 text-muted-foreground">Nenhum domínio configurado</div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableRow key={domain.id}>
                  <TableCell className="font-medium">
                    <div className="flex items-center gap-2">
                      {domain.hostname}
                      {domain.is_primary && <Badge variant="secondary">Principal</Badge>}
                    </div>
                  </TableCell>
                  <TableCell>
                    {domain.verified ? (
                    ...
                    {!domain.verified && domain.verify_token && (
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleCopyToken(domain.verify_token!)}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                      {!domain.verified && (
                        <Button variant="outline" size="sm" onClick={() => handleVerifyDomain(domain)} disabled={verifyingDomain === domain.id}>
                      ...
                      {domain.verified && (
                        <Button variant="ghost" size="sm" onClick={() => window.open(`https://${domain.hostname}`, '_blank')}>
                          <ExternalLink className="h-4 w-4" />
                        </Button>
                      )}
                      <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive" onClick={() => handleDeleteDomain(domain)}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold">Domínios</h2>
          <p className="text-muted-foreground">Gerencie os domínios do seu painel e site público</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Total de domínios</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-foreground">{domains.filter(d => d.verified).length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-amber-600">{domains.filter(d => !d.verified).length}</div>
          </CardContent>
        </Card>
      </div>

      {/* Domain Tables */}
      {renderDomainTable(adminDomains, 'Domínios do Painel (Admin)', 'Domínios para acesso ao painel administrativo')}
      {renderDomainTable(publicDomains, 'Domínios do Site Público', 'Domínios para acesso ao site público de imóveis')}
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Configure um domínio personalizado para seu painel ou site.
            </DialogDescription>
          </DialogHeader>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta ação não pode ser desfeita. O domínio {selectedDomain?.hostname} será removido.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export default DominiosContent;
